...
from tensorflow.keras.layers import Dense, Input      # For creating input and dense layers in the neural network

# 1. Load the CSV Data
# Replace 'your_data.csv' with the path to your actual CSV file containing the training data
df = pd.read_csv('data-collection\\training-data\\combined_output_2024-10-20_15-47-26.csv')
...
model.add(Dense(64, activation='relu'))  # ReLU activation helps introduce non-linearity to the model

# Add the second hidden layer with 32 neurons and 'relu' activation function
model.add(Dense(32, activation='relu'))  # Smaller number of neurons in subsequent layers is common practice
