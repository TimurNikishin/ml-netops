...
def run_iperf(bandwidth_mbps, duration):
    # Construct the iperf command with the dynamic bandwidth
    command = f"iperf3.exe -c 10.0.3.101 -u -b {bandwidth_mbps}M -t {duration}"
    print(f"Executing: {command}")
    # Use subprocess to execute the command
    subprocess.run(command, shell=True)

def generate_incremental_traffic(min_bw, max_bw, step, interval):
    try:
        current_bw = min_bw
        while current_bw <= max_bw:
            run_iperf(current_bw, interval)
...
if __name__ == "__main__":
    ...
    generate_incremental_traffic(min_bw=args.min_bw, max_bw=args.max_bw, step=args.step, interval=args.interval)
